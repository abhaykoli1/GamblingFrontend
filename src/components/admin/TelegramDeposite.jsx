...
const Deposite = () => {
    ...
    const fetchTransactionHistory = async () => {
        setLoading(true);
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/wallet/history/deposit`);
    ...
    const filteredTransactions = filterMethod
        ? transactions.filter(
            (transaction) => transaction.method.toLowerCase() === filterMethod.toLowerCase()
        )
        : transactions;
    ...
    return (
        <div className="md:min-w-4xl w-auto mx-auto px-4 py-10">
            <h2 className="text-2xl font-bold text-center mb-6 text-gray-200">Telegram Subscriptions</h2>
                            ...
                            <div className="p-4 border-b">
                                <div className="flex md:flex-row flex-col justify-between gap-4 items-center">
                                    <div className="flex justify-between items-center gap-8">
                                        <span className="font-semibold text-lg">Amount: {transaction.amount}</span>
                                        <span className="text-gray-500">{transaction.details.gameName}</span>
                                        <p>{transaction.status}</p>
                                    </div>
                                    <div className="flex gap-4 justify-between items-center text-center flex-row-reverse">
                                        <button onClick={() => toggleDetails(index)} className="text-blue-500 hover:underline">
                                            <ListCollapse />
                                        </button>
