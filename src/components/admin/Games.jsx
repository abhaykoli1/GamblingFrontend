...
const Games = () => {
  ...
  const toggleVisibility = async (gameType, newVisibility) => {
    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/api/v1/admin/game-visibility`, {
        gameType,
      ...
      setGames(prev =>
        prev.map(game =>
          game.gameType === gameType ? { ...game, isVisible: newVisibility } : game
        )
      );
    } catch (error) {
  ...
  return (
    <div className="p-6 text-gray-200 text-center">
      ...
      <table className="min-w-96 border border-gray-200 rounded">
        ...
        <tbody>
          {games.map(({ gameType, isVisible }) => (
            <tr key={gameType} className="border-t">
              <td className="px-4 py-2">{gameType}</td>
              <td className="px-4 py-2">
                <span className={`font-semibold ${isVisible ? 'text-green-600' : 'text-red-500'}`}>
