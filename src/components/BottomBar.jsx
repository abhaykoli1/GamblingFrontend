...
const BottomBar = () => {
  ...
  return (
    <nav className="min-w-screen px-4 py-2 md:border-0 border-t-1 bg-[#160003] border-amber-200">
      ...
      {/* Menu toggle */}
      <div
        className={`transition-transform duration-500 ease-in-out transform ${open ? 'translate-x-0' : '-translate-x-full'} fixed top-0 left-0 z-50 w-[14rem] h-screen bg-[#160003] text-white backdrop-blur-sm`}
      >
        <div className="flex items-start">
          ...
          <div className="p-4 text-start z-40 mt-12">
            {isAuthenticated && <div>
              ...
              {user?.role === 'admin' && (
                <Link to="/admin" className="block py-2 text-sm hover:shadow-xs shadow-red-500 focus:outline-none">
                  Admin Panel
                </Link>
              )}</div>}
            ...
            {games.map((gameKey) => {
              ...
              return (
                <Link key={info.displayName} to={info.link} className="block py-2 text-sm hover:shadow-xs shadow-red-500 focus:outline-none">
                  {info.displayName}
                </Link>
              );
            })}

            {isAuthenticated && <a onClick={logout} className="block py-2 text-sm hover:shadow-xs shadow-red-500 focus:outline-none cursor-pointer">
              Sign out
            </a>}
          </div>
        </div>
